...
- 精灵：32×32 RGB565（2KB），由 🐈 emoji 光栅化 → 降采样 → 重映射到固件橘猫 6 色板生成。
...
1. Playwright 无头 Chromium 渲染 🐈 emoji（512px、透明底截图）。
2. Pillow LANCZOS 降采样到 32×32。
3. 最近色映射到固件橘猫 6 色板：
   - 主体 `#f6c880`、深色/条纹 `#d69c58`、奶油/白 `#fce2b4`、粉 `#f29eac`、描边 `#402a1c`、眼 `#26262e`
4. 输出 `firmware/src/ui/cat_sprite.h`：`kCatSprite[1024]`（RGB565）+ `kCatMask[128]`（1-bit 透明遮罩）。
...
32×32 精灵在 320×240 上偏小；**默认 2× 放大到 64px**（nearest-neighbor，保留像素颗粒感），最终比例真机目视校准。
...
1. **6 色重映射丢失细节**：emoji 眼睛高光、腮红等可能被吞，真机目视确认，必要时加到 8 色板。
2. **精灵朝向与镜像**：侧身精灵 + `facingLeft_` 镜像后条纹/脸方向需核对。
3. **32px 偏小**：默认 2×=64px；若仍偏小，改生成 64×64 精灵（4 倍细节）。
